...
const LIVE_EVENT_ENDPOINT = 'http://localhost:4000/live-events';
...
function PastEvents() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`${LIVE_EVENT_ENDPOINT}/${eventId}`)
      .then(response => {
  ...
  const handleUrlChange = (e) => {
    e.preventDefault();
    axios.post(`${LIVE_EVENT_ENDPOINT}/${eventId}/update-url`, { url: newLiveStreamUrl })
      .then(() => {
  ...
  return (
    <Container>
      <Typography variant="h4" gutterBottom>Live Event Details</Typography>
